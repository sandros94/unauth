...
describe("defineTokenPair (h3v1)", () => {
  ...
  it("exposes underlying session managers for escape-hatch", async () => {
    ...
    const updateRes = await h.request("/force-update", {
      method: "POST",
      headers: { Cookie: jar.toString() },
    });
...
describe("optionalAuth (h3v1)", () => {
  ...
  it("passes when authenticated", async () => {
    ...
    h.router.get(
      "/feed",
      eventHandler({
        onRequest: optionalAuth(useAuth),
        handler: async (event) => {
          const { access } = await useAuth(event);
