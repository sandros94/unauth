...
try {
  ...
  const config = await client.discovery(issuer, clientId, undefined, undefined, {
  ...
  if (callbackUrl.origin + callbackUrl.pathname !== redirectUri.origin + redirectUri.pathname) {
